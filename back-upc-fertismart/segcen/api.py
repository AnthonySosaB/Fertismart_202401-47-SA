...
from config_permission.permissions.agricultor_permissions import AgricultorReadEditPermissions
...
from .models import Group, User, Profile, PermissionsGroup, LoginTrace
from .serializers import GroupSerializer, UserSerializer, ProfileSerializer, \
    PermissionGroupSerializer, LoginTraceSerializer
from .tasks import send_email_notified_new_user, send_email_notified_reset_password_user
...
class UserViewSet(viewsets.ModelViewSet):
    queryset = User.objects.exclude(id=1).order_by('-id')
    serializer_class = UserSerializer
    permission_classes = (rest_Or(SuperuserReadEditPermissions, SoporteReadEditPermissions, AgricultorReadEditPermissions),)
    ...
    def reset_password(self, request):
        ...
        try:
            ...
            data_email = {
                ...
                'link_app_fertismart': settings.FERTISMART_APP_URL,
...
class CustomObtainAuthToken(ObtainAuthToken):
    def post(self, request, *args, **kwargs):
        ...
        try:
            user_groups = user.groups.all()
            for group in user_groups:
                if group.user_manual is not None:
                    manual_file = Site.objects.get_current().domain + settings.MEDIA_URL + str(group.user_manual)
                else:
                    manual_file = None
...
class CustomRegisterUser(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if profile_data:
            ...
            profile.type_document__id = profile_data.get('type_document')
